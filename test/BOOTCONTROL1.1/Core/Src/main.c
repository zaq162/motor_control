...
int main(void)
{
  ...
  while (1)
  {
		...
		else if( SHARED->flag ==0x66)
		{
				if( flasherase_flag==1)
				{
					flasherase_flag=0;
					Flash_eraseBuf(section);
				}
				...
				if( flashwr_flag==1)
				{
				    flashwr_flag=0;
				  	...
				  	APP_address=APP_address+(uint32_t)g_decoder.len;
				}
		}
...
void Flash_WriteBuf(uint32_t addr, uint8_t *p, uint32_t len) //len 字节数
{
    ...
    for (i = 0; i < len; i += 8) {
        ...
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, addr + i, data) != HAL_OK) {
            //printf("Flash write error @ 0x%lx\r\n", addr + i);
        }
    }
		//addr=addr+i; //写地址的计算
		HAL_FLASH_Lock();
...
void Flash_eraseBuf(uint32_t  section)
{
    HAL_FLASH_Unlock();

    FLASH_EraseInitTypeDef erase = {0};
                     erase.TypeErase = FLASH_TYPEERASE_SECTORS;
    if( section==4 ) erase.Sector = FLASH_SECTOR_4;  //0x08010000 所在扇区 
	  if( section==5 ) erase.Sector = FLASH_SECTOR_5; 
		if( section==6 ) erase.Sector = FLASH_SECTOR_6; 
		if( section==7 ) erase.Sector = FLASH_SECTOR_7; 
										erase.NbSectors = 1;
										erase.VoltageRange = FLASH_VOLTAGE_RANGE_3;  // 2.7-3.6 V

    uint32_t pageErr = 0;
    HAL_StatusTypeDef status = HAL_FLASHEx_Erase(&erase, &pageErr);
