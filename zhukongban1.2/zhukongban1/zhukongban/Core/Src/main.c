...
uint8_t rx2_buffer[RX_BUFFER_SIZE];   //接高压板，串口2
uint8_t rx3_buffer[RX_BUFFER_SIZE];   //接上位机，串口3
uint8_t rx1_buffer_copy_block[RX_BUFFER_SIZE];
uint8_t rx1_buffer_copy_v[RX_BUFFER_SIZE];
uint8_t rx1_buffer_copy_x[RX_BUFFER_SIZE];
uint8_t rx3_buffer_copy[RX_BUFFER_SIZE];
...
uint8_t v_flag = 0;       //如果等于1则对应速度信号转上位机
uint8_t x_flag = 0;       //如果等于1则对应位置信号转上位机
...
// 使能空闲中断
  ...
  while (1)  //定时器定时查询，目前是延时1秒查询，不用延时较好
  {
		
		if(rx3_flag ==1)  // 上位机下发数据
		{ 
			__disable_irq(); //关总中断，保证DMA缓存区数据不会被下次串口数据改变
        uint16_t  rx3_length_copy = rx3_length;
				for (size_t i = 0; i < rx3_length_copy; ++i) 
				{
					rx3_buffer_copy[i] = rx3_buffer[i];
				}
        rx3_flag = 0;
        __enable_irq();

	
			if(rx3_buffer_copy[0] == 0x5A)  //5A是需要转发给高压板的数据
			{
							GybMessage message_422 = 
							{
								.Header = rx3_buffer_copy[0], //数据帧头
								.Addr = rx3_buffer_copy[1],  // 数据地址
								.Len = rx3_buffer_copy[2],   // 数据长度
								.Byte0 = rx3_buffer_copy[3], // 指令
								.Byte1 = rx3_buffer_copy[4], // 特殊位索引1
								.Byte2 = rx3_buffer_copy[5], // 特殊位索引 2
								.Byte3 = rx3_buffer_copy[6], // 特殊位索引 3
								.Byte4 = rx3_buffer_copy[7], // 数据字节 1
								.Byte5 = rx3_buffer_copy[8], // 数据字节 2
								.Byte6 = rx3_buffer_copy[9], // 数据字节 3
								.Byte7 = rx3_buffer_copy[10] // 数据字节 4
							};
							...
							{
								com_flag = 1;   
								HAL_UART_Transmit_DMA(&huart2, rx3_buffer_copy, rx3_length_copy);   //转发给高压板
							}	
			}

			else if(rx3_buffer_copy[0] == 0x01)  //01是需要转发给电机的数据
			{
				motor_flag =1;
				HAL_UART_Transmit_DMA(&huart1, rx3_buffer_copy, rx3_length_copy);   //转发给电机
			}
		}
		

		if(rx1_flag ==1 && motor_flag ==1 )  // 接收驱动器命令反馈的数据
		{ 
				__disable_irq(); //关总中断，保证DMA缓存区数据不会被下次串口数据改变
        uint16_t  rx1_length_copy_block = rx1_length;
				for (size_t i = 0; i < rx1_length_copy_block; ++i) 
				{
					rx1_buffer_copy_block[i] = rx1_buffer[i];
				}
        rx1_flag = 0;
				motor_flag =0;
        __enable_irq();
				
				HAL_UART_Transmit_DMA(&huart3, rx1_buffer_copy_block, rx1_length_copy_block); //把电机命令数据上传给上位机
		...
		//查询电机速度，
		 ...
		 HAL_Delay(20);  
			...
			if(rx1_flag ==1 && v_flag==1)  // 接收驱动器的数据
					{ 
							__disable_irq(); //关总中断，保证DMA缓存区数据不会被下次串口数据改变
							uint16_t  rx1_length_copy_v = rx1_length;
							for (size_t i = 0; i < rx1_length_copy_v; ++i) 
							{
								rx1_buffer_copy_v[i] = rx1_buffer[i];
							}
							rx1_flag = 0;
							 v_flag=0;
							__enable_irq();
							
							HAL_UART_Transmit_DMA(&huart3, rx1_buffer_copy_v, rx1_length_copy_v); //把电机速度数据上传给上位机
		...
		//查询电机位置，
			...
			if(rx1_flag ==1 && 	x_flag==1)  // 接收驱动器的数据
					{ 
							__disable_irq(); //关总中断，保证DMA缓存区数据不会被下次串口数据改变
							uint16_t  rx1_length_copy_x = rx1_length;
							for (size_t i = 0; i < rx1_length_copy_x; ++i) 
							{
								rx1_buffer_copy_x[i] = rx1_buffer[i];
							}
							rx1_flag = 0;
								x_flag=0;
							__enable_irq();
							
							HAL_UART_Transmit_DMA(&huart3, rx1_buffer_copy_x, rx1_length_copy_x); //把电机数据上传给上位机
