...
uint8_t fault_buffer[11]= {0x5A, 0x01, 0x08, 0x66, 0x66,
...
/*pwm	
     ...
     //计数等待
		...
		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET);   //刹车线
		*/
		//testtesttesttesttesttesttest
		...
		if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_4) == GPIO_PIN_RESET && KV85_1==1 )  //高压板的KV85,下降沿有效
		{
			KV85_1=0;
			HAL_UART_Transmit(&huart3, fault_buffer, 11 ,0xFFFF );
		}
...
{  
	if(GPIO_Pin == GPIO_PIN_14)  //探测器返回信号FPD;PE14
   {
	 ...
	 //增加一个判断
	 if(GPIO_Pin == GPIO_PIN_4)   //手闸一；PE4
   {
		pre_flag = 1;
	 }	 
	 
	 if(GPIO_Pin == GPIO_PIN_2)   //手闸二;PE2
   {
		expose_flag = 1;
	 }
	 ...
	 if(GPIO_Pin == GPIO_PIN_5)   //高压板报错，Fault。PA5
   {
	 ...
	 if(GPIO_Pin == GPIO_PIN_6)  //高压板回准备信号;PA6下降沿触发
   {
		Gybpre_flag = 1;
	 }
